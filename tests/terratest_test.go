...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-link-agg-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Link Aggregation Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/LinkAggregationPolicies/%s", moid), expectedJSONTemplate, vars)
}
